...
function doGravatar(sender) {
    ...
    if (gravCache[sender] == null) { gravCache[sender] = calcMD5(sender); }
    var mfCalcMD5 = gravCache[sender];
    ...
    setGravFace(gravURL, gCount, mfCalcMD5);

    function setGravFace(url, count, mfCalcMD5) {
	getMeta(url, function(width, height) {
	    gravFace.src = url;
	    gravFace.setAttribute("style", "display:inline");
	    document.getElementById("facesBox"+count).insertBefore(gravFace, document.getElementById("facesBox"+count).firstChild);
